...
public final class EventMap {
    ...
    public EventMap() {
        mutEventMap = new Reference2ObjectOpenHashMap<>();
        eventMap = new Reference2ObjectOpenHashMap<>();
    }

    public <T, V> void register(EventKey<T,V> key, final EventHolder<T, V> eventHolder) {
        if (eventMap.containsKey(key) || mutEventMap.containsKey(key)) {
            throw new RuntimeException("Duplicate event keys found!");
        }
        if (eventHolder instanceof MutableEventHolder) {
            mutEventMap.put(key, (MutableEventHolder<?, ?>) eventHolder);
        } else {
